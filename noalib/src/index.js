    ...
    constructor(opts = {}) {
        ...
        if (!opts.skipDefaultHighlighting) {
            ...
            this.defaultBlockHighlightFunction = (tgt) => {
                if (tgt) {
                    this.rendering.highlightBlockFace(
                        true,
                        tgt.position,
                        tgt.normal
                    );
...
function deprecateStuff(noa) {
    ...
    dep(
        noa,
        "_constants",
        "removed, voxel IDs are no longer packed with bit flags"
    );
    ver = "0.30";
...
var profile_hook = PROFILE > 0 ? makeProfileHook(PROFILE, "tick   ") : () => {};
var profile_hook_render =
    PROFILE_RENDER > 0 ? makeProfileHook(PROFILE_RENDER, "render ") : () => {};
