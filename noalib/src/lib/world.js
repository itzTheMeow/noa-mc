...
export class World extends EventEmitter {
    ...
    constructor(noa, opts) {
        ...
        this.setAddRemoveDistance(
            opts.chunkAddDistance,
            opts.chunkRemoveDistance
        );
...
var manualErr =
    "Set `noa.world.manuallyControlChunkLoading` if you need this API";
...
function findDistantChunksToRemove(world, ci, cj, ck) {
    ...
    world._chunksKnown.forEach((loc) => {
        ...
        var dk = loc[2] - ck;
        var dx = di * di + dk * dk;
        if (Math.abs(dj) <= ry && dx <= remDistSqX && dx + dj * dj <= remDistSq)
            return;
...
function invalidateChunksInBox(world, box) {
    var min = world._coordsToChunkIndexes(
        box.base[0],
        box.base[1],
        box.base[2]
    );
...
function lookForChunksToMesh(world) {
    var limit = 5;
    var numQueued =
        world._chunksToMesh.count() + world._chunksToMeshFirst.count();
...
function possiblyQueueChunkForMeshing(world, chunk) {
    ...
    var queue = nc === 26 ? world._chunksToMeshFirst : world._chunksToMesh;
...
function updateNeighborsOfChunk(world, ci, cj, ck, chunk) {
    ...
    for (var i = -1; i <= 1; i++) {
        for (var j = -1; j <= 1; j++) {
            for (var k = -1; k <= 1; k++) {
                if ((i | j | k) === 0) continue;
                var neighbor = world._storage.getChunkByIndexes(
                    ci + i,
                    cj + j,
                    ck + k
                );
...
World.prototype.report = function () {
    ...
    _report(
        this,
        "  to mesh:   ",
        this._chunksToMesh.arr.concat(this._chunksToMeshFirst.arr),
        0
    );
};
...
var profile_queues_hook = ((every) => {
    ...
    return function profile_queues_hook(state, world) {
        ...
        queues.tomesh =
            (queues.tomesh || 0) +
            world._chunksToMesh.count() +
            world._chunksToMeshFirst.count();
        queues.tomesh1 =
            (queues.tomesh1 || 0) + world._chunksToMeshFirst.count();
