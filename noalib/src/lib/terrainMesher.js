...
function TerrainMesher(noa) {
    ...
    this.meshChunk = function (
        ...
        var geomData = greedyMesher.mesh(
            voxels,
            mats,
            cols,
            ao,
...
function MeshBuilder(noa) {
    ...
    this.build = function (chunk, geomData, ignoreMaterials) {
        ...
        var matIDsUsed = Object.keys(matOffsets).sort((a, b) =>
            a < b ? -1 : 1
        );
        ...
        if (matIDsUsed.length === 1) {
            ...
            for (var matID4 of matIDsUsed) {
                ...
                new SubMesh(
                    ...
                    start * 12,
                    qct * 12, // vertex start, count - these appear to be used
                    start * 6,
                    qct * 6, // indices start, length
                    mesh
                );
                matNum++;
            }
    ...
    function getMultiMatForIDs(matIDs, scene) {
        ...
        multiMat.subMaterials = matIDs.map((matID) =>
            getTerrainMaterial(matID, false)
        );
        return multiMat;
    }
...
function GreedyMesher(noa) {
    ...
    function constructGeometryFromMasks(
        i,
        d,
        u,
        v,
    ...
    function packAOMask(data, ipos, ineg, j, k, skipReverse) {
        ...
        if (facingSolid) {
            ...
            a11 =
                a11 === 3 || solidLookup[data.get(ipos, j + 1, k + 1)] ? 3 : 2;
            a01 =
                a01 === 3 || solidLookup[data.get(ipos, j - 1, k + 1)] ? 3 : 2;
            a10 =
                a10 === 3 || solidLookup[data.get(ipos, j + 1, k - 1)] ? 3 : 2;
            a00 =
                a00 === 3 || solidLookup[data.get(ipos, j - 1, k - 1)] ? 3 : 2;
        ...
        if (skipReverse) {
            ...
            if (a11 === 1 && solidLookup[data.get(ipos, j + 1, k + 1)]) {
                a11 = 2;
            }
            if (a01 === 1 && solidLookup[data.get(ipos, j - 1, k + 1)]) {
                a01 = 2;
            }
            if (a10 === 1 && solidLookup[data.get(ipos, j + 1, k - 1)]) {
                a10 = 2;
            }
            if (a00 === 1 && solidLookup[data.get(ipos, j - 1, k - 1)]) {
                a00 = 2;
            }
            return (a11 << 6) | (a10 << 4) | (a01 << 2) | a00;
        }
        ...
        if (a11 === 1) {
            if (solidLookup[data.get(ipos, j + 1, k + 1)]) {
                ...
                !solidLookup[data.get(ineg, j, k + 1)] ||
