...
// locals
var tempVectors = [vec3.create(), vec3.create(), vec3.create()];
...
export class Camera {
    ...
    updateBeforeEntityRenderSystems() {
        // zoom update
        this._currentZoom +=
            (this.zoomDistance - this._currentZoom) * this.zoomSpeed;
    }
...
function cameraObstructionDistance(self) {
    if (!self._sweepBox) {
        ...
        self._sweepGetVoxel = self.noa.world.getBlockSolidity.bind(
            self.noa.world
        );
