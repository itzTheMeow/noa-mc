...
export function newDroppedItem(
  x: number,
  y: number,
  z: number,
  block: Block,
  ...
  let mesh = MeshBuilder.CreatePlane(
    "droppedItem",
    { size: sz },
    noa.rendering.getScene()
  );
  ...
  let id = (noa.entities as any).add(
    [x, y, z],
    sz,
    sz,
    mesh,
  ...
  (noa.entities as any).addComponent(
    id,
    (noa.entities.names as any).collideTerrain
  );
  (noa.entities as any).addComponent(
    id,
    (noa.entities.names as any).collideEntities,
    {
      callback: function (other) {
        if (other == noa.playerEntity) {
          (noa.entities as any).deleteEntity(id);
          droppedItems.splice(droppedItems.indexOf(id), 1);
